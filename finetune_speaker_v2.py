  ...
  for batch_idx, (x, x_lengths, spec, spec_lengths, y, y_lengths, speakers) in enumerate(tqdm(train_loader)):
    ...
    if rank==0:
      ...
      if global_step % hps.train.eval_interval == 0:
        ...
        if os.path.exists("/data/realAI/vits_ff/output_my"):
            utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
                                  os.path.join("/data/realAI/vits_ff/output_my", "G_latest.pth"))

            utils.save_checkpoint(net_d, None, hps.train.learning_rate, epoch,
                                  os.path.join("/data/realAI/vits_ff/output_my", "D_latest.pth"))
        if hps.preserved > 0:
          utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
          ...
          if os.path.exists(old_d):
            ...
            os.remove(old_d)
          if os.path.exists("/data/realAI/vits_ff/output_my") and global_step % (hps.train.eval_interval*10) == 0:
              utils.save_checkpoint(net_g, None, hps.train.learning_rate, epoch,
                                    os.path.join("/data/realAI/vits_ff/output_my", "G_{}.pth".format(global_step)))
              utils.save_checkpoint(net_d, None, hps.train.learning_rate, epoch,
                                    os.path.join("/data/realAI/vits_ff/output_my", "D_{}.pth".format(global_step)))
              #old_g = utils.oldest_checkpoint_path("/data/realAI/vits_ff/output_kss", "G_[0-9]*.pth",
              #                                     preserved=hps.preserved)  # Preserve 4 (default) historical checkpoints.
              #old_d = utils.oldest_checkpoint_path("/data/realAI/vits_ff/output_kss", "D_[0-9]*.pth", preserved=hps.preserved)
              #if os.path.exists(old_g):
              #    print(f"remove {old_g}")
              #    os.remove(old_g)
              #if os.path.exists(old_d):
